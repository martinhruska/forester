 ...
 * @brief  Computes integer addition
 */
class FI_isub : public RegisterAssignment
{
	/// Index of the register with the first operand
	size_t src1_;
...
public:
	...
	FI_isub(const CodeStorage::Insn* insn, size_t dst, size_t src1, size_t src2)
		: RegisterAssignment(insn, dst), src1_(src1), src2_(src2)
	{ }
	...
	virtual std::ostream& toStream(std::ostream& os) const {
		return os << "isub  \tr" << this->dstReg_ << ", r" << this->src1_
			<< ", r" << this->src2_;
	}
...
public:
	...
	FI_check(const CodeStorage::Insn* insn, bool* silentMode=nullptr)
		: VoidInstruction(insn, fi_type_e::fiCheck),
          silentMode_(silentMode)
	{
		if (silentMode == nullptr)
			*silentMode_ = false;
