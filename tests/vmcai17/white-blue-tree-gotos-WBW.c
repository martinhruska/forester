...
add_right: ALLOC_NODE(n->right, WHITE); break;
	...
	while (1)
	{
		if (n->left)
			if (n->right) 
				// both defined: choose where to go
				if (__VERIFIER_nondet_int()) goto go_blue_left;
				else goto go_blue_right;
			else
				// left defined, right not:
				// choose to go on left or add to the right
				if (__VERIFIER_nondet_int()) goto go_blue_left;
				else goto add_blue_right;
		else
			if (n->right) 
				// right defined, left not:
				// choose to go on right or add to the left
				if (__VERIFIER_nondet_int()) goto go_blue_right;
				else goto add_blue_left;
			else
				// both undefined: choose where to add
				if (__VERIFIER_nondet_int())  goto add_blue_left;
				else goto add_blue_right;

go_blue_left: n = n->left; continue;
go_blue_right: n = n->right; continue;

add_blue_left: ALLOC_NODE(n->left, BLUE); break;
add_blue_right: ALLOC_NODE(n->right, BLUE); break;
...
add_snd_left: ALLOC_NODE(n->left, WHITE); break;
add_snd_right: ALLOC_NODE(n->right, WHITE); break;
